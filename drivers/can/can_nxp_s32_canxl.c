				...
				const struct can_filter *filter)
{
	...
	/*
	 * The .frame type is only for filtering CAN XL frames, which not supported yet. CAN classic
	 * and CAN FD frames are only filtered according to their message ID, so the .frame is just
	 * set to CANEXCEL_CLASIC_FRAME, which matches both non-FD and FD frames.
	 */
