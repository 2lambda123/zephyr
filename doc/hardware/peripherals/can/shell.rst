...
* CAN ID
  ...
  * ``010``: The frame has a standard (11-bit) CAN ID of ``10h``.
  * ``00000010``: The frame has an extended (29-bit) CAN ID of ``10h``.

* Number of data bytes

  * ``[8]``: The number of data bytes in the frame, here a classic CAN frame with 8 data bytes.
  * ``[08]``: The number of data bytes in the frame, here a CAN FD frame with 8 data bytes.
...
The subcommands accepts a optional bus recovery timeout in milliseconds. If no timeout is specified,
